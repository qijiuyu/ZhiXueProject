...
public class Mp3Recorder {
	...
	public void startRecording() throws IOException {
		...
		new Thread() {
			...
			public void run() {
				isRecording = true;
				while (isRecording) {
					int bytes = audioRecord.read(buffer, 0, bufferSize);//读取音频数据至buffer中(读取缓冲区(在过度运行尚未读取读取的数据)的数据至buffer)
					if (bytes > 0) {
	...
	private void initAudioRecorder() throws IOException {
		...
		int frameSize = AudioRecord.getMinBufferSize(samplingRate,
				channelConfig, audioFormat.getAudioFormat()) / bytesPerFrame;  //返回字节为单位的创建AudioRecord的最小缓冲区大小
