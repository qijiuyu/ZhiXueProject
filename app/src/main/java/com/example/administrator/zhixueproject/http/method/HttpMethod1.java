...
public class HttpMethod1  extends BaseRequst {
     ...
     * @param handler
     */
    public static void updateLive(String postName,long postId,long postTopicId,long postWriterId,String postLivetime,int postIsFree,String postPrice,int postIsTop,String postInfo,final Handler handler) {
        Map<String, String> map = new HashMap<>();
        map.put("postName",postName);
        ...
        map.put("postIsTop",postIsTop+"");
        map.put("postInfo",postInfo);
        Http.getRetrofit().create(HttpApi1.class).updateLive(map).enqueue(new Callback<BaseBean>() {
            public void onResponse(Call<BaseBean> call, Response<BaseBean> response) {
                try {
                    sendMessage(handler, HandlerConstant1.ADD_LIVE_SUCCESS, response.body());
                }catch (Exception e){
